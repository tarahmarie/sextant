...
Extracts data for anchor case pairings and generates graphics
...
- all_thackeray_disraeli_pairs.csv: All pairs ranked by influence score
"""
...
# Case 1: Eliot → Lawrence (positive influence)
MIDDLEMARCH_ID = "1872-ENG18721"  # Middlemarch (1872)
WOMEN_IN_LOVE_ID = "1920-ENG19200"  # Women in Love (1920)

# Case 2: Thackeray → Disraeli (rivalry/contrast)
VANITY_FAIR_ID = "1848-ENG18480"  # Vanity Fair (1848) - need to verify
ENDYMION_ID = "1880-ENG18800"  # Endymion (1880) - need to verify
...
def find_author_pairs(df, source_id, target_id, source_label, target_label):
    """
    Find all pairs between two works by ELTEC ID.
    
    Args:
        df: DataFrame with all pairs
        source_id: ELTEC ID prefix for source work (e.g., "1872-ENG18721")
        target_id: ELTEC ID prefix for target work (e.g., "1920-ENG19200")
        source_label: Human-readable label (e.g., "Middlemarch")
        target_label: Human-readable label (e.g., "Women in Love")
    
    Returns:
        pairs_df: DataFrame of matching pairs with percentiles
        cross_author: DataFrame of all cross-author pairs (for reference)
    """
    print(f"\nFinding {source_label} → {target_label} pairs...")
    
    # Find pairs by ELTEC ID
    source_mask = df['source_name'].str.contains(source_id, na=False)
    target_mask = df['target_name'].str.contains(target_id, na=False)
    
    pairs = df[source_mask & target_mask].copy()
    
    print(f"Found {len(pairs)} {source_label} → {target_label} pairs")
    
    if len(pairs) == 0:
        print("WARNING: No pairs found!")
        # Try to help debug - show what IDs are available
        print("\nAvailable source IDs containing similar text:")
        similar = df[df['source_name'].str.contains(source_id[:8], na=False)]['source_name'].unique()[:5]
        for s in similar:
            print(f"  {s}")
        return None, None
    ...
    pairs['percentile'] = pairs['influence_score'].apply(
        lambda x: (cross_author['influence_score'] < x).mean() * 100
    )
    
    # Sort by influence score
    pairs = pairs.sort_values('influence_score', ascending=False)
    
    return pairs, cross_author
...
def find_eliot_lawrence_pairs(df):
    ...
    """
    print("\n" + "=" * 70)
    print("STEP 4a: FINDING ELIOT → LAWRENCE PAIRS")
    print("=" * 70)
    
    return find_author_pairs(
        df, 
        MIDDLEMARCH_ID, 
        WOMEN_IN_LOVE_ID,
        "Middlemarch",
...
def find_thackeray_disraeli_pairs(df):
    """
    Find all Thackeray -> Disraeli pairs (Vanity Fair -> Endymion)
    """
    print("\n" + "=" * 70)
    print("STEP 4b: FINDING THACKERAY → DISRAELI PAIRS")
    print("=" * 70)
...
def print_top_pairs(pairs, source_label, target_label, n=20):
    """
    Print top pairs with percentiles
    """
    print(f"\n--- TOP {n} {source_label.upper()} → {target_label.upper()} CHAPTER PAIRS ---\n")
    
    for i, (_, row) in enumerate(pairs.head(n).iterrows(), 1):
        src_ch = row['source_chapter']
        tgt_ch = row['target_chapter']
        print(f"{i:3}. {source_label} ch{src_ch:>2} → {target_label} ch{tgt_ch:>2}  "
              f"Score: {row['influence_score']:.4f}  "
              f"Pctl: {row['percentile']:.2f}%  "
              f"Hap: {row['hap_jac_dis_raw']:.4f}  "
              f"Align: {row['al_jac_dis_raw']:.4f}  "
...
def generate_percentile_distribution(pairs, cross_author, source_label, target_label, output_suffix, output_dir):
    """
    Generate histogram showing where top pairs fall
    """
    ...
    ax.axvline(x=top_pair['influence_score'], color='crimson', linestyle='--', linewidth=2.5,
               label=f'Top {source_label}→{target_label} ({top_pair["percentile"]:.1f}th pctl)')
    
    ax.set_xlabel('Influence Score (z-normalized, weighted)', fontsize=12)
    ax.set_ylabel('Number of Text Pairs', fontsize=12)
    ax.set_title(f'Distribution of Influence Scores: Cross-Author Pairs\n{source_label} → {target_label}', 
                 fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'percentile_distribution_{output_suffix}.png'), dpi=300)
    plt.savefig(os.path.join(output_dir, f'percentile_distribution_{output_suffix}.pdf'))
    print(f"Saved percentile distribution chart ({output_suffix})")
    plt.close()


def save_results(el_pairs, td_pairs, contributions, weights, output_dir):
    """
    Save results to files
    """
    # Save Eliot-Lawrence pairs
    if el_pairs is not None:
        el_pairs.to_csv(os.path.join(output_dir, 'all_eliot_lawrence_pairs.csv'), index=False)
        print(f"Saved {len(el_pairs)} Eliot-Lawrence pairs to CSV")
    
    # Save Thackeray-Disraeli pairs
    if td_pairs is not None:
        td_pairs.to_csv(os.path.join(output_dir, 'all_thackeray_disraeli_pairs.csv'), index=False)
        print(f"Saved {len(td_pairs)} Thackeray-Disraeli pairs to CSV")
    ...
    with open(os.path.join(output_dir, 'anchor_case_results.txt'), 'w') as f:
        ...
        f.write("=" * 60 + "\n")
        f.write("CASE 1: ELIOT → LAWRENCE (Positive Influence)\n")
        f.write("Middlemarch (1872) → Women in Love (1920)\n")
        f.write("=" * 60 + "\n\n")
        ...
        f.write("\n" + "=" * 60 + "\n")
        f.write("CASE 2: THACKERAY → DISRAELI (Rivalry/Contrast)\n")
        f.write("Vanity Fair (1848) → Endymion (1880)\n")
        f.write("=" * 60 + "\n\n")
        
        if td_pairs is not None:
            f.write("TOP 10 PAIRS:\n")
            f.write("-" * 50 + "\n")
            for i, (_, row) in enumerate(td_pairs.head(10).iterrows(), 1):
                f.write(f"{i:2}. Vanity Fair ch{row['source_chapter']:>2} → Endymion ch{row['target_chapter']:>2}  "
                       f"Pctl: {row['percentile']:.2f}%\n")
        else:
            f.write("No pairs found.\n")
...
def main():
    ...
    print("=" * 70)
    print("\nAnalyzing two cases:")
    print("  1. Eliot → Lawrence (positive influence)")
    print("  2. Thackeray → Disraeli (rivalry/contrast)")
    ...
    # First, let's discover the correct ELTEC IDs for Thackeray and Disraeli
    print("\n" + "=" * 70)
    print("DISCOVERING ELTEC IDs")
    print("=" * 70)
    
    # Find Thackeray works
    thackeray_mask = df['source_author_name'].str.contains('Thackeray', case=False, na=False)
    if thackeray_mask.any():
        thackeray_works = df[thackeray_mask]['source_name'].unique()
        print(f"\nThackeray works found:")
        for w in sorted(set(['-'.join(x.split('-')[:2]) for x in thackeray_works]))[:10]:
            print(f"  {w}")
    
    # Find Disraeli works
    disraeli_mask = df['target_author_name'].str.contains('Disraeli', case=False, na=False)
    if disraeli_mask.any():
        disraeli_works = df[disraeli_mask]['target_name'].unique()
        print(f"\nDisraeli works found:")
        for w in sorted(set(['-'.join(x.split('-')[:2]) for x in disraeli_works]))[:10]:
            print(f"  {w}")
    ...
    # === CASE 1: Eliot → Lawrence ===
    el_pairs, cross_author = find_eliot_lawrence_pairs(df)
    
    if el_pairs is not None:
        print_top_pairs(el_pairs, "Middlemarch", "WiL", n=20)
        generate_percentile_distribution(el_pairs, cross_author, "Middlemarch", "Women in Love", 
                                         "eliot_lawrence", OUTPUT_DIR)
    
    # === CASE 2: Thackeray → Disraeli ===
    td_pairs, _ = find_thackeray_disraeli_pairs(df)
    
    if td_pairs is not None:
        print_top_pairs(td_pairs, "Vanity Fair", "Endymion", n=20)
        generate_percentile_distribution(td_pairs, cross_author, "Vanity Fair", "Endymion",
                                         "thackeray_disraeli", OUTPUT_DIR)
    
    # Generate variable contribution chart (same for both)
    generate_variable_contribution_chart(contributions, OUTPUT_DIR)
    
    # Save results
    save_results(el_pairs, td_pairs, contributions, weights, OUTPUT_DIR)
